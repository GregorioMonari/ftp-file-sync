export default class BinaryNumberFormatter{


    static getNumberType(n:any){
        var string="ciao";
        var number=1;


        if(typeof n == typeof string){
            let string:string=n.toLowerCase()
    ...
    static hex2bin(n:string,size:number){
        var clean="";
        if(n.endsWith("H")||n.endsWith("h")||n.startsWith("0x")){
            clean=n.slice(0,n.length-1);            
        }else{
            clean=n;
        }
    ...
    static bin2decUnsigned(binString:string){
        var out=0;
        for(let i=0; i<binString.length; i++){
            var bit=parseInt(binString[binString.length-i-1])
            var weight=Math.pow(2,i)
            out=out+(bit*weight)
        }
        return out
    }
    static dec2binUnsigned(decimal:number,size:number){
    ...
    static dec2binSigned(decimal:number,size:number){
        if(decimal<0){
            var mod=decimal*-1
            ...
            var temp=this.bin2decUnsigned(notN)
            temp=temp+1;
